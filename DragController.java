    ...
    private void createHandlers() {
        ...
        commitPositionOnRelease = event -> {
            if (cycleStatus != INACTIVE) {
                ...
                target.setLayoutX(Math.max(Math.min(event.getSceneX(),
                                                    target.getParent().getLayoutBounds().getMaxX()),
                                           target.getParent().getLayoutBounds().getMinX())
                                  - mouseOffsetFromNodeZeroX);
                target.setLayoutY(Math.max(Math.min(event.getSceneY(),
                                                    target.getParent().getLayoutBounds().getMaxY()),
                                           target.getParent().getLayoutBounds().getMinY())
                                  - mouseOffsetFromNodeZeroY);
